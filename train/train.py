...
#Added imports by AT
from dataset.dataloader import LibriSpeechDataset, get_data_loader, speech_collate_fn
from datasets import load_dataset
from utils.consts import SYMBOLS
...
def train(args):
    ...
    #Instantiating core model components
    ...
    #AT Imp
    if args.sample:
        print("[INFO] Sample mode active: using hf-internal-testing/librispeech_asr_demo")
        hf_train = load_dataset("hf-internal-testing/librispeech_asr_demo", split="train")
    ...
    train_ds = LibriSpeechDataset(hf_train)
    train_dl = get_data_loader(train_ds, args.batch_size, shuffle=True, num_workers=2)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            PAD_ID = 28  # from SYMBOLS
            token_mask = (input_ids != PAD_ID).to(device)

            #mel specs come as [B, T, M]; AudioEncoder expects [B, 1, T, M]
            ...
            #Compute loss only on the corrupted posisions
            loss = torch.nn.functional.mse_loss(
                predicted_embeddings[corrupt_mask], 
                x_0[corrupt_mask]
            )

            #Holy shit, the backward pass... finally!
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample", action="store_true", help="Run a small sample for quick GPU test using 73-entry demo dataset")
